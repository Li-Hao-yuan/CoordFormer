...
class ParamEncoder(nn.Module):
    '''
    [6890x3,64,64] -> [1,256,512]
    '''
    def __init__(self,final_channel=1):
        super(ParamEncoder,self).__init__()
        self.head_linear = nn.Linear(6809*3, 128, bias=False)
...
class VOMP(Base):
    ...
    def head_forward(self,out,params_map=None):
        ...
        params_map = self.param_enc(params_map) # [6890x3,64,64] -> [256,512]
        dec_output,camera_map,center_map = self.trans(out,params_map) 
        param_output = self.param_dec(dec_output) # [1,256,512]->[142,64,64] ->(SMPL model)-> vertice
...
# 还差一步 怎么从pose转到vertice -> 再导入一个SMPL model
