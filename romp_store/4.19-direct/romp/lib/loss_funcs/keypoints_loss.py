...
def batch_kp_2d_l2_loss(real, pred, weights=None):
    if np.any(np.isnan(pred.cpu().data.numpy())):

        print(real)
        print(pred)
        exit()

        mask = np.isnan(pred.cpu().data.numpy()) 
        pred[ np.expand_dims(mask,0)  ] = -2
    ...
    #here
    # if len(loss) == 1:
        # loss = torch.cat((loss,loss,loss),dim=0).cuda()
       
    #     loss = torch.tensor([1,1,1]).to(torch.float32).cuda()
...
def calc_pampjpe(real, pred, sample_wise=True,return_transform_mat=False):
    ...
    # print("real",real.shape) [1or3,14,1]
    # print("pred",pred.shape) [3,14,1]
