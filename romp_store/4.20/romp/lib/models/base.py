    ...
    def feed_forward(self, meta_data):
        ...
        if args().video:
            ...
            if args().predict:
                ...
                confidence = torch.ones( (params.shape[0],64,1) ).cuda()
                redundancy = torch.zeros( (params.shape[0],64,1) ).cuda()
                dec_input = torch.cat( (params,person_centers,confidence,redundancy),dim=2 ).cuda() #[batch_size,64,80]
                
                # 位移
                zero_head = torch.zeros( (1,64,80) ).cuda()
