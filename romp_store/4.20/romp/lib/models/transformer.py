...
class Transformer(nn.Module):
    def __init__(self):
        ...
        self.projection = nn.Linear(args().emb_size, 132+10+2+1, bias=False) # 一层就ok了
        self.tanh = nn.Tanh()
        self.sig = nn.Sigmoid()

    def predict_forward(self,enc_inputs):
        '''
        enc_inputs: [batch_size, src_len]
        '''        
        dec_inputs = torch.tensor([])
        zero_dec_input = torch.zeros(1,64,80)
        
        for round in range( enc_inputs.shape[0] ):
            pre_enc_outputs, pre_enc_self_attns = self.pre_encoder(enc_inputs[:round+1])
            enc_outputs,enc_self_attns = self.encoder(pre_enc_outputs)    
            
            if dec_inputs.shape[0] > 0:
                dec_inputs = torch.cat( (dec_inputs , zero_dec_input),dim=0 )
            else:
                dec_inputs = zero_dec_input
            
            dec_outputs, dec_self_attns, dec_enc_attns = self.decoder(dec_inputs, enc_inputs, enc_outputs)
            dec_inputs = dec_outputs
            
            if round == enc_inputs.shape[0] - 1:
                camera_map = self.cameraer(pre_enc_outputs) # [3,64,64]
                dec_logits = self.projection(dec_outputs) # dec_logits: [batch_size, tgt_len, tgt_vocab_size]
                
                dec_logits[:,:,:132]   = self.sig(dec_logits[:,:,:132])
                dec_logits[:,:,132:142] = self.sig(dec_logits[:,:,132:142])
                dec_logits[:,:,142]    = self.sig(dec_logits[:,:,142])
                dec_logits[:,:,143]    = self.sig(dec_logits[:,:,143])
                dec_logits[:,:,144]    = self.sig(dec_logits[:,:,144])
            
        return dec_logits, camera_map
        
    def train_forward(self, enc_inputs, dec_inputs):
        '''
        enc_inputs: [batch_size, src_len]
        dec_inputs: [batch_size, tgt_len]
        '''
    ...
    def forward(self, enc_inputs, dec_inputs=None):
        if dec_inputs == None:
            dec_logits, camera_map = self.predict_forward(enc_inputs)
        else:
            dec_logits, camera_map = self.train_forward(enc_inputs,dec_inputs)
