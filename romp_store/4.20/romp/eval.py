...
def val_result(self, loader_val, evaluation = False):
    ...
    for iter_num, meta_data in enumerate(loader_val):
        ...
        if args().data_slide_len > 1 and args().batch_size==1:
            for key in meta_data.keys():
                # imgpath [silde,1,83]
                if key in ['imgpath','data_set']:
                    meta_data[key] = [ meta_data[key][i][0] for i in range(args().data_slide_len) ]
                    continue

                meta_data[key] = meta_data[key].squeeze(0)

        if args().data_slide_len > 1 and args().batch_size >1:
            for key in meta_data.keys():
                if key in ['imgpath','data_set']: continue

                shape = meta_data[key].shape
                meta_data[key] = meta_data[key].view(args().data_slide_len*args().batch_size,*shape[2:])
        ...
        for ds in set(outputs['meta_data']['data_set']):
            ...
            #     real_3d = outputs['meta_data']['kp_3d'][val_idx:int(val_idx+args().data_slide_len)]
            #     real_3d = real_3d.contiguous().cuda()
            ...
            # if predicts.shape[0] > args().data_slide_len:
            #     predicts = predicts[:args().data_slide_len]
        ...
        if iter_num % self.val_print_size == 0:
            ...
            if self.save_visualization_on_img:
                if args().data_slide_len > 1:
                    outputs['meta_data']['data_set'] = outputs['meta_data']['data_set'][0]
