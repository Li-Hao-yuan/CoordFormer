...
class ScaledDotProductAttention(nn.Module):
    ...
    def forward(self, Q, K, V, attn_mask):
        ...
        if scores.shape[2] > 256:
            scores = scores[:,:,:256,:]
        if scores.shape[3] > 256:
            scores = scores[:,:,:,:256]
...
class Encoder(nn.Module): 
    ...
    def forward(self, enc_inputs):
        ...
        enc_outputs = enc_inputs.view(batch_size,args().src_len,args().emb_size) # [32,128,128] -> [512,1024]
...
class Maper(nn.Module):
    def __init__(self,final_channel=1,actvite_func=None):
        super(Maper,self).__init__()
        self.actvite_func = actvite_func
        self.pool2d = nn.MaxPool2d(2)
        self.layers = nn.ModuleList([
            MaperLayer(16,8),
            MaperLayer(8,4),
            MaperLayer(4,final_channel)
            ])
        
        
    def forward(self,enc_outputs):
        '''
        enc_outputs: [batch_size, src_len, d_model] [1024,1024] -> [64,128,128] -> [1,64,64]
        '''
        batch_size = enc_outputs.shape[0]
        feature_map = enc_outputs.view(batch_size,16,128,128)
        for layer in self.layers:
            feature_map = layer(feature_map)
        
        feature_map = self.pool2d(feature_map)
        
        if self.actvite_func is not None:
            if self.actvite_func == nn.Tanh():
                feature_map[:,1,:,:] = self.actvite_func(feature_map[:,1,:,:])
                feature_map[:,2,:,:] = self.actvite_func(feature_map[:,2,:,:])
            else:
                feature_map = self.actvite_func(feature_map)
...
class Transformer(nn.Module):
    def __init__(self):
        ...
        self.head_layer = MaperLayer(32,16)
        self.encoder = Encoder()
        self.decoder = Decoder()
        self.pointer = Maper(1,nn.Sigmoid())
        self.cameraer = Maper(3,nn.Tanh())
    ...
    def forward(self, enc_inputs, dec_inputs):
        ...
        enc_inputs = self.head_layer(enc_inputs) # [64,128,128]
        enc_outputs, enc_self_attns = self.encoder(enc_inputs) # [256,1024]

        heatmap = self.pointer(enc_outputs) # [1,64,64]
        
        # print(heatmap.shape)
        camera_map = self.cameraer(enc_outputs) # [3,64,64]

        # dec_outpus: [batch_size, tgt_len, d_model], dec_self_attns: [n_layers, batch_size, n_heads, tgt_len, tgt_len], dec_enc_attn: [n_layers, batch_size, tgt_len, src_len]
        dec_outputs, dec_self_attns, dec_enc_attns = self.decoder(dec_inputs, enc_inputs, enc_outputs)

        return dec_outputs, camera_map, heatmap#,enc_self_attns, dec_self_attns, dec_enc_attns
...
if __name__ == "__main__":
    ...
    dec_input = torch.zeros((2,256,1024))
