...
class Loss(nn.Module):
    ...
    def _calc_detection_loss(self, outputs, meta_data):
        ...
        if args().force_centermap_loss:
            detect_loss_dict['CenterMap'] = self.ver_loss_func(outputs['center_map'], meta_data['centermap'].to(device)) #((centermaps-centermaps_gt)**2).sum(-1).sum(-1).mean(-1) #
        elif all_person_mask.sum()>0 :
    ...
    def _calc_keypoints_loss(self, outputs, meta_data):
        kp_loss_dict, error = {'P_KP2D':0, 'MPJPE':0, 'PAMPJPE':0,'Vertices':0}, {'3d':{'error':[], 'idx':[]},'2d':{'error':[], 'idx':[]}}
        if 'pred_vertice' in outputs:
            kp_loss_dict['Vertices'] = self.ver_loss_func(outputs['pred_vertice'],outputs['real_vertice'])
