...
class Image_base(Dataset):
    ...
    def get_item_single_frame(self,index):
        ...
        for i in range(args().data_slide_len):
            ...
            info = self.get_image_info(index+i)
            ...
            input_data = {
                'image': dst_image,
                'image_org': org_image,
                'full_kp2d': full_kp2ds,
                'person_centers':person_centers,
                'subject_ids':subject_ids,
                'centermap': centermap.data.numpy(),
                'kp_3d': kp3d,
                'params': params,
                'valid_masks':valid_masks,
                'offsets': offsets,
                'rot_flip': [rot, flip],
                'all_person_detected_mask':[all_person_detected_mask],
                'imgpath': info['imgpath'],
                'data_set': info['ds']}

            if args().learn_2dpose:
                input_data.update({'heatmap':heatmap})
            if args().learn_AE:
                input_data.update({'AE_joints':AE_joints})

            tem_input.append(input_data)

        if args().data_slide_len > 1:
            total_input_data = {}
            for key in tem_input[0].keys():
                if key in ['imgpath','data_set']:
                    total_input_data[key] = [ tem_input[i][key] for i in range(len(tem_input)) ]
                else:
                    total_input_data[key] = np.array([ tem_input[i][key] for i in range(len(tem_input)) ])

            total_input_data['image'] = torch.from_numpy(total_input_data['image']).float()
            total_input_data['image_org'] = torch.from_numpy(total_input_data['image_org']),
            total_input_data['full_kp2d'] = torch.from_numpy(total_input_data['full_kp2d']).float(),
            total_input_data['person_centers'] = torch.from_numpy(total_input_data['person_centers']).float(), 
            total_input_data['subject_ids'] =torch.from_numpy(total_input_data['subject_ids']).long(),
            total_input_data['centermap'] = torch.from_numpy(total_input_data['centermap']).float(),
            total_input_data['kp_3d'] = torch.from_numpy(total_input_data['kp_3d']).float(),
            total_input_data['params'] = torch.from_numpy(total_input_data['params']).float(),
            total_input_data['valid_masks'] =torch.from_numpy(total_input_data['valid_masks']).bool(),
            total_input_data['offsets'] = torch.from_numpy(total_input_data['offsets']).float(),
            total_input_data['rot_flip'] = torch.from_numpy(total_input_data['rot_flip']).float(),
            total_input_data['all_person_detected_mask'] = torch.from_numpy(total_input_data['all_person_detected_mask']).bool(),

            if args().learn_2dpose:
                total_input_data['heatmap'] = torch.from_numpy(total_input_data['heatmap']).float()
            if args().learn_AE:
                total_input_data['AE_joints'] = torch.from_numpy(AE_joints['heatmap']).long()
