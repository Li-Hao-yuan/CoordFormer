...
class EncoderHead(nn.Module):
    def __init__(self):
        ...
        self.end = nn.Sequential(
            ...
            nn.Linear(256, 512, bias=False)  #[1,256,512]
        )
...
class Encoder(nn.Module): 
    '''
    x:[32,128,128] -> [256,512]
    [32,128,128]->[1,256,256]->[1,256,512]
    '''
    ...
    def forward(self, enc_inputs):
        ...
        enc_outputs = enc_inputs.view(batch_size,args().src_len,args().emb_size) # [1,256,512] -> [256,512]
...
class Maper(nn.Module):
    def __init__(self,final_channel=1):
        ...
        self.actvite_func = nn.LayerNorm([64,64])
        self.pool2d = nn.MaxPool2d(2)
        self.layers = nn.ModuleList([
            MaperLayer(8,16),
            MaperLayer(16,8),
            MaperLayer(8,final_channel)
            ])
    ...
    def forward(self,enc_outputs):
        ...
        feature_map = enc_outputs.view(batch_size,8,128,128)
        for layer in self.layers:
...
class Transformer(nn.Module):
    def __init__(self):
        ...
        self.pointer = Maper(1)
        self.cameraer = Maper(3)

    def forward(self, enc_inputs, dec_inputs, last_frame=True):
        '''
        ...
        enc_inputs = self.head(enc_inputs) # [32,128,128] -> [1,256,512]
        enc_outputs, enc_self_attns = self.encoder(enc_inputs) # [256,512]
...
if __name__ == "__main__":
    ...
    dec_input = torch.zeros((2,256,512))

    dec_output = model(enc_input,dec_input)[0]
    print(dec_output.shape) # [256,512]