...
class Encoder(nn.Module): 
    ...
    def forward(self, enc_inputs):
        ...
        enc_outputs = enc_inputs.view(batch_size,args().src_len,args().emb_size) 
...
class Maper(nn.Module):
    def __init__(self,final_channel=1,activate_func=None):
    ...
    def forward(self,enc_outputs):
        '''
        enc_outputs: [batch_size, src_len, d_model] [512,128] -> [4,128,128] -> [1,64,64]
        '''
        batch_size = enc_outputs.shape[0]
        feature_map = enc_outputs.view(batch_size,4,128,128)
        for layer in self.layers:
            feature_map = layer(feature_map)
        
        feature_map = self.pool2d(feature_map)
        if self.activate_func is not None:
            feature_map = self.activate_func(feature_map)
...
class Transformer(nn.Module):
    def __init__(self):
        ...
        self.enc_head = EncoderHead() # [32,128,128] -> [1,512,512]
        self.projection = nn.Linear(args().src_len, args().emb_size, bias=False)

        self.encoder = Encoder()
        self.decoder = Decoder()
        self.pointer = Maper(1,nn.Sigmoid())
        self.cameraer = Maper(3,None)
    ...
    def forward(self, enc_inputs, dec_inputs):
        ...
        enc_inputs = self.enc_head(enc_inputs) # [32,128,128] -> [1,512,512]
        enc_inputs = self.projection(enc_inputs) # [1,512,512] -> [1,512,128]
...
if __name__ == "__main__":
    ...
    dec_input = torch.zeros((2,512,128))
