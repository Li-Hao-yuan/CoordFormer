...
class ParamEncoder(nn.Module):
    ...
    def __init__(self,final_channel=1):
        ...
        self.projection = nn.Linear(512, 128, bias=False)
...
class ParamDecoder(nn.Module):
    ...
    def __init__(self,final_channel=142):
        ...
        self.projection = nn.Linear(128, 512, bias=False)
        
    def forward(self,enc_outputs):

        batch_size = enc_outputs.shape[0]
        feature_map = enc_outputs.view(batch_size,1,512,128)
        feature_map = self.projection(feature_map)
        ...
        return feature_map.view(batch_size,142,64,64)
...
class VOMP(Base):
    ...
    def head_forward(self,out,params_map):
        ...
        params_map = torch.squeeze(params_map,dim=1) # [b,512,128]

        dec_output,camera_map,center_map = self.trans(out,params_map)
        param_output = self.param_dec(dec_output) # [batch_size,142,64,64]

        # param_output[:,:132,:,:] = self.sig(param_output[:,:132,:,:])
        # param_output[:,132:,:,:] = self.sig(param_output[:,132:,:,:])
