...
class NTAttention(nn.Module):
    ...
    def forward(self, input_Q, input_K, input_V, attn_mask):
        ...
        Q = self.W_Q(input_Q).view(batch_size, -1, args().trans_heads, self.dk).transpose(1,2)  # Q: [batch_size, n_heads, len_q, d_k]
        K = self.W_K(input_K).view(batch_size, -1, args().trans_heads, self.dk).transpose(1,2)  # K: [batch_size, n_heads, len_k, d_k]
        V = self.W_V(input_V).view(batch_size, -1, args().trans_heads, self.dk).transpose(1,2)  # V: [batch_size, n_heads, len_v(=len_k), d_v]
        ...
        context = context.transpose(1, 2).reshape(batch_size, -1, args().trans_heads * self.dk) # context: [batch_size, len_q, n_heads * d_v]
...
class Decoder(nn.Module):
    ...
    def forward(self, dec_inputs, enc_outputs):
        ...
        dec_self_attn_pad_mask = torch.zeros((batch_size,dec_inputs.shape[1],dec_inputs.shape[1])).to(bool) # [batch_size, tgt_len, tgt_len]
        
        dec_self_attn_subsequence_mask = get_attn_subsequence_mask(dec_inputs) # [batch_size, tgt_len, tgt_len]
        dec_self_attn_mask = torch.gt((dec_self_attn_pad_mask + dec_self_attn_subsequence_mask), 0) # [batch_size, tgt_len, tgt_len]

        #dec_enc_attn_mask = get_attn_pad_mask(dec_inputs, enc_inputs) # [batc_size, tgt_len, src_len]
        dec_enc_attn_mask = torch.zeros((batch_size,dec_inputs.shape[1],dec_inputs.shape[1])).to(bool) # [batc_size, tgt_len, src_len]
...
class Transformer(nn.Module):
    ...
    def forward(self, enc_inputs, dec_inputs):
        ...
        dec_outputs, dec_self_attns, dec_enc_attns = self.decoderw(enc_outputs,enc_inputs)
        dec_outputs, dec_self_attns, dec_enc_attns = self.decoderl(dec_outputs,enc_outputs)
