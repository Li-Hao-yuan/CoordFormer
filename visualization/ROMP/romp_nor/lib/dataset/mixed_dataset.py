...
class MixedDataset(torch.utils.data.Dataset):
    def __init__(self, **kwargs):
        ...
        self.shuffle_list = np.arange(self.total_length)
    ...
    def __getitem__(self, index):
        ...
        #print(annots['subject_ids'].shape) #(64,)

        index = self.shuffle_list[index]
        dataset_id = 0
        annots = self.datasets[dataset_id][index]

        if args().data_slide_len > 1:

            #i don't konw why
            for key in annots.keys():
                if type(annots[key]) == tuple:
                    annots[key] = annots[key][0]
