...
class Internet(Dataset):
    def __init__(self, file_list=[], **kwargs):
        ...
        self.annot = np.load(j3d_path, allow_pickle=True)['j3d'][()]
    ...
    def get_item_single_frame(self, index):
        ...
        for i in range(args().data_slide_len):
            ...
            input_data.update({'other_kp3d':self.annot[str(index)]})
            # print(type(self.annot[str(index)]))
        ...
        total_input_data['other_kp3d'] = np.array([tem_input[i]['other_kp3d'] for i in index_list])
        total_input_data['other_kp3d'][0] =  np.ones((64,54,3))*-2
        total_input_data['other_kp3d'] = torch.from_numpy(total_input_data['other_kp3d']).float()
