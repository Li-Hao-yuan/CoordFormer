...
class Visualizer(object):
    ...
    def visulize_result(self, outputs, meta_data, show_items=['org_img', 'mesh'], vis_cfg=default_cfg, save2html=True, **kwargs):
        ...
        img_names = np.array(meta_data['imgpath'])[img_inds_org] # 6

        # print(meta_data['image'].cpu().numpy().astype(np.uint8)[img_inds_org].shape)

        if len(meta_data['image'].shape)==5: # data slide length -> 6,8,512,512,3
            org_imgs = meta_data['image'][:,0,:,:,:].cpu().numpy().astype(np.uint8)[img_inds_org]
