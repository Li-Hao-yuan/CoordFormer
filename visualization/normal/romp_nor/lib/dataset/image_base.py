...
class Image_base(Dataset):
    ...
    def get_item_single_frame(self,index):
        ...
        for i in range(args().data_slide_len):
            # print(scale, rot, flip)

            info = self.get_image_info(index+i*args().data_gate) # 起点

            if i == 0:
                random_index = np.random.randint(info['vmask_2d'][:,0].sum())

            img_info = process_image(info['image'], info['kp2ds'], augments=(scale, rot, flip), is_pose2d=info['vmask_2d'][:,0], multiperson=mp_mode,random_index=random_index)
            if img_info is None:
            ...
            if i < args().data_slide_len-1:
                ...
                total_input_data = else_data
            tem_input.append(input_data)

        if args().data_slide_len > 1:
            for key in ['image', 'image_org']:

                dict_info = []
                target_video_name = tem_input[-1]['imgpath'].split('/')[-2]
                for i in range(len(tem_input)):
                    index = len(tem_input) - i - 1
                    if tem_input[index]['imgpath'].split('/')[-2] == target_video_name:
                        dict_info.append(tem_input[index][key])
                    else:
                        dict_info.append(dict_info[-1])
                dict_info.reverse()
                total_input_data[key] = np.array(dict_info)
            ...
            index_list = [args().data_slide_len - 1]
            index_list.extend([i for i in range(index_list[0])])
            total_input_data['other_kp3d'] = torch.from_numpy(np.array([tem_input[i]['other_kp3d'] for i in index_list])).float()
