...
def get_mesh_stand_on_img(pose,betas,trans,meta_data,reorganize_idx,interactive_vis):
    ...
    per_img_verts_list = T_smpl_outs['verts']  # [1,1,6890,3]
    mesh_trans = trans.unsqueeze(0)  # [1,1,3]
    # org_imgs [1,512,512,3]

    #[1,6890,3],[1,3],
    rendered_imgs = visualizer.plot_multi_meshes_batch(per_img_verts_list, trans, meta_data, \
                    reorganize_idx, interactive_show=interactive_vis, rotate_frames=[0])
...
def gen_mesh_stand_on_img():
    ...
    img_path = 'D:/3D/lihaoyuan/vomp/dataset/h36m/images/S1_Directions 1_0_0.jpg'
    ...
    info['trans'][0][2] -= 2.2  # 大小
    # info['trans'][0][1] -= 0.6  # 上
    info['trans'][0][0] -= 2.  # 左
    # print(info['trans'])

    meta_data = {}
    meta_data['image'] = torch.tensor(img).cuda()
    meta_data['imgpath'] = [img_path]

    render_img = get_mesh_stand_on_img(pose=pose, betas=np.reshape(info['betas'], (1, 10)), trans=torch.from_numpy(info['trans'][:1]).cuda(), \
                             meta_data=meta_data,reorganize_idx=torch.tensor([0]),interactive_vis=False)

    get_video_from_img('stand_img',lambda x:int(x.replace('.jpg','').split('_')[-1]))


    # print(render_img.shape)
    # cv2.imshow("0",render_img[0])
    # cv2.waitKey(0)
    #
    # cv2.imwrite('D:/3D/lihaoyuan/vomp/dataset/trash/mesh_stand_on_img.jpg', render_img)


def get_video_from_img(filename,lambda_f):
    import os
    import cv2
    from tqdm import tqdm

    image_path = 'D:/3D/lihaoyuan/vomp/dataset/trash/'+filename+'/'
