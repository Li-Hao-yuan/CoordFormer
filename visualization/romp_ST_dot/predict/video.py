...
class Video_processor(Image_processor):
    ...
    def process_video(self, video_file_path):
        ...
        internet_loader = self._create_single_data_loader(dataset='pw3d', train_flag=False, shuffle=False)
        counter.start()
        ...
        for test_iter,meta_data in enumerate(internet_loader):
            if test_iter >= 240: break
            # print("meta_data['image']",meta_data['image'].shape)
            meta_data['image'] = meta_data['image'].squeeze(0)
            # print("meta_data['image']", meta_data['image'].shape)
            ...
            if self.show_mesh_stand_on_image:
                print("outputs['reorganize_idx']",outputs['reorganize_idx'])
            ...
            if self.save_visualization_on_img:
                ...
                if 'centermap' in results_dict.keys():
                    center_root = 'D:\\3D\\lihaoyuan\\predict\\ROMP\\demo\\centermap\\'
                    center_path = center_root + img_names[0].split("\\")[-1]
                    img = np.array(results_dict['centermap']['figs'])[0]
                    cv2.imwrite(center_path, img)
                    # print(np.array(results_dict['centermap']['figs']).shape)
...
if __name__ == '__main__':
    args().configs_yml = 'configs/video.yml'
    args().GPUS = 0
