...
class Base(nn.Module):
    ...
    def feed_forward(self, meta_data):
        ...
        x = self.backbone(meta_data['image'].contiguous().cuda().view(args().batch_size*args().data_slide_len,512,512,3))
        # x = x.view(args().batch_size*args().data_slide_len,512,512,3)
