...
def parse_args(input_args=None):
    ...
    parser.add_argument('--interactive_vis', action='store_true',
                        help='whether to show the results in an interactive mode')
    parser.add_argument('--show_largest_person_only', action='store_true',
                        help='whether to only show the results of the largest person in the image')
    parser.add_argument('--show_mesh_stand_on_image', action='store_true',
                        help='whether to show the estimated meshes standing on the image')
    parser.add_argument('--surrounding_camera', action='store_true',
                        help='whether to surround the camera around the origin')
    parser.add_argument('--soi_camera', type=str, default='far',
                        help='camera mode of show_mesh_stand_on_image: far / close')
    parser.add_argument('--make_tracking', action='store_true', help='whether to make tracking')
    parser.add_argument('--temporal_optimization', action='store_true',
                        help='whether to optimize the temporal smoothness')
    parser.add_argument('--save_dict_results', action='store_true',
                        help='whether to save the predictions to a dict (.npz)')
    parser.add_argument('--save_visualization_on_img', action='store_false',
                        help='whether to rendering the mesh back to image, which is time consuming')
    ...
    parser.add_argument('--renderer', type=str, default='pyrender', help='character: pytorch3d / pyrender')
    parser.add_argument('-f', type=str, default=None,
                        help='do nothing, just to deal with the invalid input args from jupyter notebook')
    parser.add_argument('-video', type=bool, default=True, help='if VOMP?')
    ...
    mode_group.add_argument('--FOV', type=float, default=60,
                            help='The camera field of view (eular angle) used for visualization')
    mode_group.add_argument('--focal_length', type=float, default=443.4,
                            help='Default focal length, adopted from JTA dataset')
    ...
    train_group.add_argument('--GPUS', type=str, default='1', help='gpus')
    train_group.add_argument('--batch_size', default=32, help='batch_size', type=int)
    ...
    train_group.add_argument('--force_centermap_loss', type=bool, default=False,
                             help='whether force to get loss of centermap')

    transformer_group = parser.add_argument_group(title='transformer settings')
    transformer_group.add_argument('--src_len', type=int, default=256, help="enc input length")
    transformer_group.add_argument('--tgt_len', type=int, default=256, help="dec input length")
    transformer_group.add_argument('--emb_size', type=int, default=512, help="embedding size")
    transformer_group.add_argument('--forward_dim', type=int, default=2048, help="feedforward dimension")
    transformer_group.add_argument('--dk', type=int, default=64, help="dimension of K(Q)")
    transformer_group.add_argument('--dv', type=int, default=64, help="dimension of V")
    transformer_group.add_argument('--trans_layer', type=int, default=6, help="number of encoder and decoder Layer")
    transformer_group.add_argument('--trans_heads', type=int, default=8, help="number of heads in Multi-Head Attention")
    transformer_group.add_argument('--predict', type=bool, default=False, help="if transformer predict mode")
    ...
    model_group.add_argument('--hrnet_pretrain', type=str,
                             default=os.path.join(project_dir, 'trained_models/pretrain_hrnet.pkl'))
    model_group.add_argument('--resnet_pretrain', type=str,
                             default=os.path.join(project_dir, 'trained_models/pretrain_resnet.pkl'))
    ...
    eval_group.add_argument('--val_print_size', default=64, help='valiation print size', type=int)
    ...
    # dataset setting:
    dataset_group.add_argument('--dataset_rootdir', type=str, default=os.path.join(root_dir, 'dataset/'),
    ...
    dataset_group.add_argument('--data_slide_len', type=int, default=1, help='dataset slide length')
    dataset_group.add_argument('--to_frame', type=bool, default=True, help='video to frame')
    ...
    smpl_group.add_argument('--smpl_model_path', type=str, default=os.path.join(model_dir, 'parameters'),
                            help='smpl model path')
    smpl_group.add_argument('--smpl_J_reg_h37m_path', type=str,
                            default=os.path.join(model_dir, 'parameters', 'J_regressor_h36m.npy'),
                            help='SMPL regressor for 17 joints from H36M datasets')
    smpl_group.add_argument('--smpl_J_reg_extra_path', type=str,
                            default=os.path.join(model_dir, 'parameters', 'J_regressor_extra.npy'),
                            help='SMPL regressor for 9 extra joints from different datasets')

    smpl_group.add_argument('--smpl_uvmap', type=str, default=os.path.join(model_dir, 'parameters', 'smpl_vt_ft.npz'),
    ...
    smpl_group.add_argument('--mesh_cloth', type=str, default='031',
                            help='pick up cloth from the wardrobe or simplely use a single color')

    smpl_group.add_argument('--nvxia_model_path', type=str, default=os.path.join(model_dir, 'characters', 'nvxia'),
                            help='path to nvxia model')
    ...
    with open(config_yml_path, 'rb') as file:
        configs_update = yaml.safe_load(file)
