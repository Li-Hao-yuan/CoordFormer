...
def process_image(originImage, full_kp2ds=None, augments=None, is_pose2d=True, multiperson=False,random_index=0):
    ...
    if augments is not None:
        ...
        if not multiperson and is_pose2d.sum()>0:
            max_len = len(np.where(np.array(is_pose2d))[0])
            if random_index >= max_len : random_index = max_len - 1

            kps_vis = full_kp2ds[np.where(np.array(is_pose2d))[0][random_index]]
