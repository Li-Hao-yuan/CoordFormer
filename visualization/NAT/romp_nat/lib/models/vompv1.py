...
class VOMP(Base):
    ...
    def train_forward(self,out,params_map):
        ...
        param_dec = self.param_dec(dec_output.view(args().batch_size,args().data_slide_len,64,-1))
    ...
    def predict_forward(self,out):
        ...
        params_loop = torch.zeros(batch_size,64,54,3).cuda()


        output = self.train_forward(out,params_loop)
        return output
