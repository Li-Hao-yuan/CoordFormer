...
from ROMP.romp.lib.utils.util import save_result_dict_tonpz
        ...
        if self.global_count % (6 * self.print_freq) == 0 or self.global_count == 50:
            ...
            if self.save_visualization_on_img:
                ...
                if args().data_slide_len > 1:
                    for ds_name in set(outputs['meta_data']['data_set'][0]):
                ...
                train_vis_dict = self.visualizer.visulize_result(outputs, outputs['meta_data'],
                                                                 show_items=['org_img', 'mesh', 'pj2d', 'centermap'], \
    ...
    def train_epoch(self, epoch):
        ...
        for iter_index, meta_data in enumerate(self.loader):
            ...
            if self.global_count % self.test_interval == 0 or self.global_count == self.fast_eval_iter:  # self.print_freq*2
                ...
                np.save(os.path.join(self.model_save_dir, 'optimizer.npy'), self.optimizer.state_dict()["param_groups"])
                # torch.save(self.optimizer.state_dict()["param_groups"],os.path.join(self.model_save_dir, 'optimizer.pth'))
